  ...
  return (
    <PageTemplate
      ...
      lead={
        <Card className="surface-panel p-6">
          <div className="flex flex-col gap-4">
            <div className="flex items-center gap-2 rounded-2xl border border-border/60 bg-muted/25 p-1">
              {viewModes.map((mode) => (
                <Button
                  key={mode.key}
                  variant={mode.active ? "default" : "ghost"}
                  size="sm"
                  className={cn("flex-1 rounded-xl", !mode.active && "text-muted-foreground")}
                >
                  {mode.label}
                </Button>
              ))}
          ...
          <PageSection
            ...
            action={
              <div className="flex items-center gap-2">
                <Button variant="ghost" size="icon" className="border border-border/60">
            ...
            <div className="grid gap-5 lg:grid-cols-2">
              {events.map((event) => (
                <Card
                  key={event.id}
                  className={cn(
                    "surface-panel space-y-4 p-5 transition hover:border-primary/40",
                    event.id === "mapTraining" && "border-primary/30",
                  )}
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="space-y-2">
                      <div className="flex flex-wrap items-center gap-2">
                        <Badge
                          variant="outline"
                          className={cn("border text-xs", event.badgeClasses)}
                        >
                          {event.tag}
                        </Badge>
                  ...
                  {event.id === "mapTraining" && event.hasAttendees ? (
      ...
      floatingAction={
        <Button
          type="button"
          className="fixed bottom-24 right-6 z-40 h-14 w-14 shadow-xl shadow-primary/30 lg:hidden"
          size="icon"
        >
